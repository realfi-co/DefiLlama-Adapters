...
module.exports = {
  ...
  start: '2026-06-17',
  methodology: 'TVL corresponds to USDrf locked in the RealFi staking vault on Cardano, read from the vault script address. RealFi operates a two-token structure: USDrf is the stable token, pegged to one US dollar, and sUSDrf is the staked token. Holders deposit USDrf and receive sUSDrf, whose exchange rate is the vault settled USDrf backing divided by the circulating sUSDrf supply, so it rises as the backing portfolio earns and falls when it loses. Each distribution splits the yield or loss between staked and unstaked supply according to an explicit parameter supplied in the signed distribution transaction, and under RealFi yield distribution policy losses are allocated to sUSDrf ahead of unstaked USDrf. USDrf held outside the vault is reported separately under RealFi USDrf.',
  cardano: {
    tvl,
  },
}
